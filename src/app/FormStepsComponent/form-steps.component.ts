...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, BasicInfoComponent, GuaranteesComponent, CreditsComponent, DocumentsComponent, ReviewComponent],
  template: `
    <div class="container py-3">
      ...
      <div class="mb-3">
        <h6 class="fw-bold mb-1">{{ activeStep()?.title }}</h6>
        <p class="text-muted small">{{ activeStep()?.description }}</p>
      </div>

      <div class="mt-2">
        @switch (activeKey()) {
          @case ('basic') { <app-basic-info /> }
          @case ('guarantees') { <app-guarantees /> }
          @case ('credits') { <app-credits /> }
      ...
      <div class="d-flex justify-content-between align-items-center mt-4">
        <button class="btn btn-link" type="button" (click)="cancel.emit()">Cancelar</button>
        <div class="d-flex gap-2">
          <button class="btn btn-outline-secondary" type="button" (click)="onBack()">Voltar</button>
          <button class="btn btn-primary" type="button" (click)="onNext()">Continuar</button>
        </div>
      </div>
    </div>
  `
...
export class FormStepsComponent {
  @Input() steps: FormStep[] = [
    { key: 'basic', title: 'Informações básicas', description: 'Dados de identificação do trade.' },
    { key: 'guarantees', title: 'Garantias', description: 'Detalhes de garantias.' },
    { key: 'credits', title: 'Créditos', description: 'Condições e valores.' },
    { key: 'documents', title: 'Documentos', description: 'Envio e validação de documentos.' },
    { key: 'review', title: 'Revisão', description: 'Conferência final antes de enviar.' },
  ];
  ...
  onBack() {
    ...
    const prev = this.steps.slice(0, idx).reverse().find(s => !s.disabled);
    if (prev) {
